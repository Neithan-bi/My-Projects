...
void viewAttendance(struct Queue *q, char name[]) {
    int found = 0;
    for (int i = q->front; i <= q->rear; i++) {
        if (strcmp(q->data[i].name, name) == 0) {
            struct Student *s = &q->data[i];
...
void listStudents(struct Queue *q) {
    if (isEmpty(q)) {
...
void searchStudentMenu(struct Queue *q) {
    if (isEmpty(q)) {
        printf("\nNo students to search.\n");
        return;
    }
    listStudents(q);
    printf("Select student number to view: ");
    int choice;
    scanf("%d", &choice);
    getchar();

    int index = q->front + choice - 1;
    if (index >= q->front && index <= q->rear) {
        viewAttendance(q, q->data[index].name);
    } else {
